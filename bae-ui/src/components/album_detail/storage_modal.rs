...
pub fn StorageModal(
    ...
    on_transfer_to_managed: EventHandler<()>,
...
fn StorageStatusSection(
    ...
    let (icon, label, detail) = if managed_locally && managed_in_cloud {
        (
            rsx! {
            ...
            "Local + Cloud",
            format!(
                "Files stored locally and in cloud • {} files • {}",
                file_count,
                format_file_size(total_size),
            ),
        )
    } else if managed_locally {
        (
            rsx! {
                HardDriveIcon { class: "w-5 h-5 text-blue-400" }
            },
            "Managed locally",
            format!(
                "Files in library storage • {} files • {}",
                file_count,
                format_file_size(total_size),
            ),
        )
    } else if managed_in_cloud {
        (
            rsx! {
        ...
        (
            ...
            "No storage",
            format!("{} files • {}", file_count, format_file_size(total_size)),
        )
    };
    ...
    rsx! {
        div { class: "p-4 bg-gray-700/50 rounded-lg space-y-2",
            div { class: "flex items-center gap-3",
                div { class: "p-2 rounded-lg bg-blue-500/20", {icon} }
                div {
                    div { class: "text-sm font-medium text-white", {label} }
                    div { class: "text-xs text-gray-400 mt-0.5", {detail} }
                }
            }
        }
    }
...
fn TransferActionsSection(
    ...
    // Show "Copy to library" for unmanaged releases
    let can_copy_to_managed = is_unmanaged;
    // Show "Eject" for managed releases
    let can_eject = managed_locally;

    if !can_copy_to_managed && !can_eject {
        return rsx! {};
    }

    rsx! {
        div { class: "space-y-3",
            if can_copy_to_managed {
                div {
                    div { class: "text-sm font-medium text-gray-300", "Copy to library" }
                ...
                button {
                    ...
                    onclick: move |_| on_transfer_to_managed.call(()),
                    div { class: "p-1.5 rounded bg-gray-600/50",
                        HardDriveIcon { class: "w-4 h-4 text-gray-300" }
                    }
                    div { class: "flex-1",
                        div { class: "text-sm text-white", "Managed local storage" }
                        div { class: "text-xs text-gray-400", "Copy files into the library" }
                    }
