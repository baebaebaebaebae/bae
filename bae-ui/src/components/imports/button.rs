...
pub fn ImportsButtonView(
    ...
    let has_in_progress = imports
        .iter()
        .any(|i| i.status == ImportStatus::Preparing || i.status == ImportStatus::Importing);
    let has_failed = imports.iter().any(|i| i.status == ImportStatus::Failed);
